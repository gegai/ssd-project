...
class SSHinfo(object):
    ...
    def Cmd(self,ip,user,password,port,cmd):
        ...
        ssh.connect(hostname=ip, port=port, username=user, password=password,timeout=2)
    ...
    def save_to_db(self,card_list,ip,id):
        ...
        if card_info is not None:
            ...
            for item in card_info:
                ...
                # print card_info1
                card_info1=SSHinfo().operate_cardinfo(card_info1)
    ...
    '''根据指定id获取数据'''
    def execute_ssh_host(self,ip,username,password,port,id_db,ssh_ip,ssh_username,ssh_password,ssh_port):
        # ret_json = {'status': 10000}
        ret_json = {}
        try:


            cmd='./get_hostinfo.sh'+" "+ip+" "+username+" "+password+" "+port
            print 'cmd_start'+ ssh_ip+'     '+ip


            result=SSHinfo().Cmd( ssh_ip,ssh_username,ssh_password,int(ssh_port),cmd)
            print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
            print result
            if result['status']=='10001':
                ...
                result=SSHinfo().save_to_db(card_list,ip,id_db)
...
# result=SSHgroup().execute_ssh_host(56892)
# print result
